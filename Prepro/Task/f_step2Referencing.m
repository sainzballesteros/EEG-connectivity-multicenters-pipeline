...
%INPUTS:
%pathStep0 = Path of the .set of the step 0 (marked .set)
%nameStep0 = Name of the .set of the step 0 (marked .set)
%isReferenced = True if the .json said that the EEGs were already referenced. False otherwise
%pathStep1 = Path of the .mat containing the vector of channel indexes to exclude, as well as the labels of those channels. (Empty by default)
%nameStep1 = Name of the .mat containing the vector of channel indexes to exclude, as well as the labels of those channels. (Empty by default)
...
if ~isempty(pathStep1) && ~isempty(nameStep1)
    ...
    for i = 1:length(badChanIdxs)
        ...
        if ~strcmp(chanLbls{badChanIdxs(i)}, badChanLbls{i})
            ...
            disp('Do not correspond to the channel labels of the marked .set (Step 0) at the following path:');
