function status = f_updateParametersTxtTask(paramToUpdate, params)
%Description:
...
if ~ (strcmp(paramToUpdate, 'filterAndResample') || strcmp(paramToUpdate, 'newSR') || ...
        ...
        strcmp(paramToUpdate, 'onlyBlinks') || strcmp(paramToUpdate, 'eventName') || ...
        strcmp(paramToUpdate, 'epochRange') || strcmp(paramToUpdate, 'jointProbSD') || ...
        strcmp(paramToUpdate, 'kurtosisSD') || strcmp(paramToUpdate, 'baselineRange'))
    status = 0;
    fprintf('%s is not a valid parameter to update. For f_mainPreproTask the valid parameters are: \n', paramToUpdate);
    disp('filterAndResample, reref_REST, onlyBlinks, eventName, epochRange, jointProbSD, kurtosisSD, baselineRange');
    return
end
...
if isempty(initialIdx)
    ...
    %If the parameter is eventName or epochRange, add that information  
    elseif strcmp(paramToUpdate, 'eventName')
        %Transfroms the contents of newEventName from a cell to a string separated by a comma
        strEventNames = '';
        tempVal = params.(paramToUpdate);
        for jEvent = 1:length(tempVal)
            strEventNames = strcat(strEventNames, char(string(tempVal{jEvent})), ', ');
        end
        strEventNames(end) = [];
        fprintf(fileID, 'Step 6 [Epoch definition]: \n');
        fprintf(fileID, '\t -%s = {%s} (1) \n ', 'eventName', strEventNames);
    elseif strcmp(paramToUpdate, 'epochRange')
        fprintf(fileID, '\t -%s = [%.3f, %.3f] (1) \n \n', 'epochRange', params.(paramToUpdate));
        %Works okay because 'epochRange' is run just after 'eventName'. A more elegant implementation would be recommended
...
%If the desired parameter already existed, and it appears only once, modify it
    ...
    %Corrects the conversion to char for 'eventName'
    elseif strcmp(paramToUpdate, 'eventName')
        paramValue = '';
        tempVal = params.(paramToUpdate);
        for jEvent = 1:length(tempVal)
            paramValue = strcat(paramValue, char(string(tempVal{jEvent})), ', ');
        end
        paramValue(end) = [];
    ...
    %Corrects the conversion to char if two numbers are given as a vector []
    if strcmp(paramToUpdate, 'freqRange') || strcmp(paramToUpdate, 'epochRange') || ...
        strcmp(paramToUpdate, 'baselineRange') || strcmp(paramToUpdate, 'eventName')
    ...
    if isempty(initialNumParam)
        ...
        if strcmp(paramToUpdate, 'filterAndResample') && strcmp(paramValue, 'true')
            ...
            elseif endsWith(paramValue, '}') && ~startsWith(paramValue, '{')
                %Had to add this special case for ranges, due to formatting problems with regexp
                finalTxt = sprintf('%s; {%s (1) %s', txtParams(1:finalNumParam+initialIdx-2), paramValue, ...
                    txtParams(finalNumParam+initialIdx-1:end));
            else
    ...
    if strcmp(paramToUpdate, 'filterAndResample') && strcmp(paramValue, 'true') && ~isempty(initialNumParam)
        status0 = f_updateParametersTxtTask('newSR', params);
        status1 = f_updateParametersTxtTask('freqRange', params);
